        ...
        if mode == 'single':
            ...
            if head_part == None:
                ...
                head = torch.index_select(
                    self.entity_embedding,
                    dim=0,
                    index=head_part.view(-1).cuda()
                ).view(batch_size, negative_sample_size, -1)

            relation = torch.index_select(
                self.relation_embedding,
                dim=0,
                index=tail_part[:, 1].cuda()
            ).unsqueeze(1)
